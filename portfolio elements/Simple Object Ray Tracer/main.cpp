...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    const double b = ((double)2 * (ray_direction.dot(ray_origin - sphere_center)));
    const double c = (ray_origin - sphere_center).dot(ray_origin - sphere_center) - pow(sphere_radius,2);
    
    const double quad_formula_under_sqrt = pow(b, 2) - ((double)4 * a * c);

    if (quad_formula_under_sqrt < 0){
        return -1;
    }else{
        //set the correct intersection point, update p to the correct value
        double const t = ((double)(-1) * b - sqrt(quad_formula_under_sqrt)) / ((double)2 * a);

        const Vector3d p_out = ray_origin + t * ray_direction;
        p = p_out;
        const Vector3d N_out = (p - sphere_center).normalized();
        N = N_out;
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Matrix3d A_mat;    
    A_mat << ray_direction, ((double)(-1)*pgram_u), ((double)(-1)*pgram_v);

    const Vector3d b = pgram_origin - ray_origin;
    Vector3d x = A_mat.colPivHouseholderQr().solve(b);

    if (x[0] < 0 || x[1] < 0 || x[1] > 1 ||  x[2] < 0 || x[2] > 1){
        return -1;
    }

    //set the correct intersection point, update p and N to the correct values
    const Vector3d p_out = ray_origin + x[0] * ray_direction;
    p = p_out;
    const Vector3d N_out = (double)(-1)* pgram_u.cross(pgram_v).normalized();
    N = N_out;
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d p, N;
    if (find_nearest_object(ray_origin, ray_direction, p, N) != -1){
        return false;
    }
    return true;
}
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    if (nearest_object < 0 || max_bounce == 0) //if ray hits nothing or if ray has bounced max_bounce times.
    {
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        const Vector3d p_offset = p + (.00001) * Li;
        ...
        const Vector3d Vi = (ray_origin - p).normalized();
    ...
    if (nearest_object == 4) //sphere 4 is reflective i think lol
    {
        refl_color = Vector4d(0.5, 0.5, 0.5, 0);
    }
    // Compute the color of the reflected ra y and add its contribution to the current point color.
    // use refl_color
    const Vector3d ray_direction_normalized = ray_direction.normalized();
    Vector3d reflection_ray_direction = (ray_direction_normalized - (double)2 * ray_direction_normalized.dot(N) * N).normalized();
    
    const Vector3d p_offset = p + (.00001) * N; //delta offest to avoid colision
    
    Vector4d reflection_color = refl_color.cwiseProduct(shoot_ray(p_offset, reflection_ray_direction, max_bounce-1));
