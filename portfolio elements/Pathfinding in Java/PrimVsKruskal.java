...
public class PrimVsKruskal{
	...
	*/
	static boolean PrimVsKruskal(double[][] G){
		...
		/* ... Your code here ... */
		EdgeWeightedGraph graph = new EdgeWeightedGraph(G.length, 0);
		for (int i = 0; i < G.length; i++){
			for(int j = 0; j < G.length; j++){
				if (G[i][j] != 0.0){
					Edge tempEdge = new Edge(i,j,G[i][j]);
					graph.addEdge(tempEdge);
				}
			}
		}
		PrimMST prim = new PrimMST(graph);
		KruskalMST krus = new KruskalMST(graph);

		if (prim.weight() != krus.weight()){
			return false;
		}
		
		for (Edge e1: prim.edges()){
